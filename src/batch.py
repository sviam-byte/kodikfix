...
def run_single_subject(
    ...
    *,
    ...
    ricci_n_jobs: Optional[int] = None,
) -> dict:
    ...
    def _run_job(index: int, matrix_file: Path, density: float) -> dict:
        ...
        try:
            metrics = run_single_subject(
                ...
                seed=seed,
                eff_sources_k=eff_sources_k,
                # Избегаем nested parallelism: при batch-параллели ORC считаем в 1 поток.
                ricci_n_jobs=1 if int(n_jobs) != 1 else None,
            )
